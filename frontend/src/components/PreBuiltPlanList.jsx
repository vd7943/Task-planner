...
const PreBuiltPlanList = () => {
  ...
  useEffect(() => {
    ...
    const url = `https://task-planner-k097.onrender.com/plan/get-plan/${role}/${authUser.userType}`;
  ...
  const handleOptPlan = async (planId) => {
    try {
      const response = await axios.post(
        "https://task-planner-k097.onrender.com/plan/opt-plan",
        {
          userId: authUser._id,
          planId,
        },
  ...
  return (
    <div className="p-6 lg:mx-auto h-full pt-16 md:pt-4 w-full xl:w-[960px]">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-semibold text-gray-200">Plan List</h2>
        {authUser.role === "Admin" && (
          <Link to="/add-plan">
